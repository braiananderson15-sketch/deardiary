...
describe("setup lifecycle", () => {
  it("previews before confirmation and declining creates no files or backups", async () => {
    ...
    expect(question).toContain("MCP command: npx -y deardiary mcp");
    expect(question).toContain("- deardiary entry: absent");
    expect(question).toContain('+ command: "npx"; args: ["-y", "deardiary", "mcp"]');
  ...
  it("preserves unrelated config, copies skills, and creates non-clobbering backups", async () => {
    ...
    expect(result.exitCode, result.output).toBe(0);
    expect(result.output).toContain("run 'npx -y deardiary doctor'");
    ...
    expect(codex).toContain('args = ["-y", "deardiary", "mcp"]');
  ...
  it("repairs malformed launchers and missing Codex startup timeout for package-runner survival", async () => {
    const paths = makePaths();
    write(
      paths.claudeConfig,
      `${JSON.stringify({ mcpServers: { deardiary: { type: "stdio", command: "deardiary", args: ["serve"], env: {} } } }, null, 2)}\n`,
    );
    write(paths.codexConfig, `[mcp_servers.deardiary]\ncommand = "deardiary"\nargs = ["serve"]\n`);
    write(
      paths.openCodeConfig,
      `${JSON.stringify({ mcp: { deardiary: { type: "local", command: ["deardiary", "serve"], enabled: true } } }, null, 2)}\n`,
    );
...
describe("uninstall lifecycle", () => {
  ...
  it("prints the exact CLI instruction and requires the stable typed phrase for full wipe", async () => {
    ...
    expect(wiped.output).toContain("Remove the CLI after this command exits: npm rm -g deardiary");
    ...
    expect(cliOnly.output).toContain(
      "Remove the CLI after this command exits: npm rm -g deardiary",
    );
...
describe("doctor and startup benchmark", () => {
  ...
  it("returns nonzero for an unhealthy database or failed MCP startup", async () => {
    ...
    expect(result.output).toContain("run 'npx -y deardiary setup'");
